...
"""Tests for [`NotifierService`][terok_clearance.runtime.installer.NotifierService] — desktop-bridge unit installer."""
...
class TestNotifierServiceInstall:
    """``NotifierService.install`` writes the single notifier unit under XDG."""
    ...
    def test_writes_unit_with_bin_path_substituted(
        ...
        with patch.object(_install, "_daemon_reload"):
            dest = NotifierService.install(Path("/usr/local/bin/terok-clearance-notifier"))
    ...
    def test_is_idempotent(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
        with patch.object(_install, "_daemon_reload"):
            first = NotifierService.install(Path("/a/terok-clearance-notifier"))
            second = NotifierService.install(Path("/a/terok-clearance-notifier"))
...
class TestNotifierServiceUninstall:
    """``NotifierService.uninstall`` removes the notifier unit + reloads systemd."""
    ...
    def test_tolerates_missing_unit(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
        with patch.object(_install, "_daemon_reload"):
            NotifierService.uninstall()  # no assertion needed — just must not raise


class TestOutdatedSummaryCoversNotifier:
    """``outdated_summary`` reports stale notifier units independently of hub/verdict."""
