...
[`EventSubscriber`][terok_clearance.EventSubscriber] turns the event
stream from one [`ClearanceClient`][terok_clearance.ClearanceClient]
into calls on an injected
[`Notifier`][terok_clearance.notifications.protocol.Notifier]: a block
arrives, the operator sees a popup with Allow/Deny actions, clicks
route back to the hub as a ``Verdict``.  Live-block dedup and
shield-down popup tracking live here because they're presentation
concerns; identity resolution does not — the shield reader resolves
the orchestrator-supplied dossier at emit time and ships it on every
event, so the renderer just reads it.

[`MultiSocketSubscriber`][terok_clearance.MultiSocketSubscriber] fans
several per-container hubs into one notifier: it owns an
[`EventSubscriber`][terok_clearance.EventSubscriber] per hub socket and
reconciles the set as containers come and go.
"""
...
class MultiSocketSubscriber:
    ...
    *socket_glob* (default
    ...
    Args:
        ...
        rescan_interval_s: Seconds between glob rescans.  Must be
            strictly positive.  The default (2.0) balances startup
            latency for a freshly-spawned supervisor against background
            polling cost when the host is idle.
    """
    ...
    async def _rescan_loop(self) -> None:
        ...
        Cancellation is the normal teardown path: `stop` cancels this
        task, so the ``CancelledError`` from ``asyncio.sleep`` is meant
        to propagate out unhandled — `stop` awaits and suppresses it.
        """
        while not self._stopping:
            await asyncio.sleep(self._rescan_interval_s)
            # _stopping is flipped by stop() from another task, so this
            # guard is reachable even though mypy can't see the mutation.
            if self._stopping:
