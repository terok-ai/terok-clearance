...
In the per-container-supervisor model the hub and verdict server are
composed in-process by the supervisor (one of each per container) via
[`HubService`'s now-deleted ancestor][terok_clearance.ClearanceHub] and
[`VerdictServer`][terok_clearance.VerdictServer].  Operator UIs that
used to talk to a single global hub socket now multiplex across the
per-container sockets exposed by [`MultiSocketSubscriber`][terok_clearance.MultiSocketSubscriber].
"""
